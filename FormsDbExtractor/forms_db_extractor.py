...
def read_sp_args():
    """ To read the input values for SP arguments
...
def create_db_connection(host, db_name, user_name, password):
    ...
    '''
    try:
        con_str = f'DRIVER=SQL Server;SERVER={host};DATABASE={db_name};UID={user_name};PWD={password}'
...
def generate_parquet_file(script, file_name, conn):
    ...
    try:
        ...
        parquet_info = False, 0, f'failed to execute the script : {script}'
        return parquet_info
...
def remove_parquet_file(file_name):
    ...
    try:
        ...
        logging.info(f"Failed to remove the parquet file : {file_name}")
...
def parquet_job(job_info):
    ...
    script = script.replace('CAST(', 'CAST((')

    if expected_row_count == 0:
        logging.info(
            f"expected row count is zero for the script id : {job_info['result'][0]}  filename : {job_info['result'][2]}")
        logging.info('end')
        return False

    logging.info(
        f'script id: {job_info["result"][0]} and filename: {file_name}')
    conn = create_db_connection(
        job_info['host'], config.FORMS_DB_NAME, job_info['username'], job_info['password'])

    logging.info(
        f'generating parquet file for filename: {file_name} script: {script}')
    ...
    logging.info(
        f'insert the row into csv file for script id : {job_info["result"][0]}  filename : {file_name}  run status: {run_status}')
    audit_row = audit_row_dic(None, script, run_status,
...
def run_jobs(sp_results, forms_db_username, forms_db_password, audit_filename):
    ''' To execute the jobs asynchronously
    Args:
        sp_results: splitted row lists based on max job count
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for sp_result in sp_results:
            ...
            for row in zip(job_info, executor.map(parquet_job, job_info)):
                if row[1]:
                    logging.info(
                        f"parquet generation completed for the script id : {row[0]['result'][0]}  filename : {row[0]['result'][2]} ")
                else:
                    logging.info(
                        f"parquet generation failed for the script id : {row[0]['result'][0]}  filename : {row[0]['result'][2]} ")
...
def main():
    ...
    # Read forms secrets
    forms_db_username = config.FORMS_DB_USER_NAME
    forms_db_password = read_secret_from_key_vault(
        config.KEY_VAULT_NAME, config.FORMS_KEY_VAULT_SECRET_NAME_DB_PASSWORD)

    # Get results from SP
    sp_results = get_list_extract_queries(config.FORMS_DB_SERVER_HOST,
    ...
    job_details = split_jobs(sp_results, config.MAX_JOB_COUNT)
