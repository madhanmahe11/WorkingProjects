...
def parquet_job(job_info):
    ...
    conn = create_db_connection(job_info['host'], config.FORM_DB_NAME, job_info['username'], job_info['password'])
        ...
        for result in results:
            ...
            for row in result:
                local_dic = {   'result': row,
                                'host': config.FORM_DB_SERVER_HOST, 
...
if __name__ == "__main__":
    ...
    # Read form secrets
    form_db_username = read_secret_from_key_vault(config.KEY_VAULT_NAME, config.FORM_KEY_VAULT_SECRET_NAME_DB_USER_NAME)
    ...
    #To execute the jobs asynchronously
    run_jobs(job_details, form_db_username, form_db_password)